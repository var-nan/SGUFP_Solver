...
TEST_F(DDTest, TestReduceLayer){
	for (int i = 1; i < restrictedDD.tree.size()-2; i++){
		...
		if (networkPtr->hasStateChanged[i])ASSERT_EQ(s2, 1);
...
protected:
	...
	DDSubRootTest() {
		...
		MEASURE_EXECUTION_TIME(exactDD.build(root), message);
...
TEST_F(DDSubRootTest, Test1) {
	...
	auto ind = networkPtr->processingOrder.size()-restrictedDD.getGlobalPosition();
...
TEST(DDNew, TestCutSet) {
	...
	DD restricted2{networkPtr,RESTRICTED};
	cout << "States: "; for (auto s: ddnode.states) {cout << s << " ";} cout << endl;
	auto cutset2 = restricted2.build(ddnode);
	...
	auto [_,arcId] = networkPtr->processingOrder[cutnode.globalLayer];
	auto networkArc = networkPtr->networkArcs[arcId];
	if (networkPtr->hasStateChanged[cutnode.globalLayer]) {
		cout << "State changed ";
		cout << networkPtr->stateUpdateMap.at(cutnode.globalLayer).size() << endl;
	}
